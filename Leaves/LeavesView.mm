...
- (void)setUpLayersForViewingMode {
	NSLog(@"- (void)setUpLayersForViewingMode {");
...
- (id)initWithFrame:(CGRect)frame {
	NSLog(@"- (id)initWithFrame:(CGRect)frame {");
...
- (void) getImages {
	...
	else
	{
		if (currentPageIndex <= numberOfPages) {
			...
			//NSLog(@"CPI %i", currentPageIndex);

			
			if (isTurningRight) {
				if (lastTurnDirection == 1) {
					////NSLog(@"YOUR TURNING LEFT AGAIN...");
					leftPage.contents = (id)[pageCache cachedImageForPageIndex:1];
					topPage.contents = (id)[pageCache cachedImageForPageIndex:2];
					topPageReverseImage.contents = (id)[pageCache cachedImageForPageIndex:3];
					bottomPage.contents = (id)[pageCache cachedImageForPageIndex:4];
					

					[pageCache cachedImageForPageIndex:5];
					[pageCache cachedImageForPageIndex:6];

					
					//topPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex- 2];
					//leftPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex-3];
					//topPageReverseImage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex - 1];
					//bottomPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex];
				}
				else {
					////NSLog(@"YOUR TURNING LEFT THE FIRST TIME!...");
					leftPage.contents = (id)[pageCache cachedImageForPageIndex:1];
					topPage.contents = (id)[pageCache cachedImageForPageIndex:2];
					topPageReverseImage.contents = (id)[pageCache cachedImageForPageIndex:3];
					bottomPage.contents = (id)[pageCache cachedImageForPageIndex:4];
				...
				if (lastTurnDirection == -1) {
					////NSLog(@"YOUR TURNING RIGHT AGAIN...");
					//topPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex-2];
					//leftPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex-3];
					//topPageReverseImage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex - 1];
					//bottomPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex];
					
					[pageCache cachedImageForPageIndex:1];
					[pageCache cachedImageForPageIndex:2];
					
					leftPage.contents = (id)[pageCache cachedImageForPageIndex:3];
					topPage.contents = (id)[pageCache cachedImageForPageIndex:4];
					topPageReverseImage.contents = (id)[pageCache cachedImageForPageIndex:5];
					bottomPage.contents = (id)[pageCache cachedImageForPageIndex:6];
					

				}
				else {
					////NSLog(@"YOUR TURNING RIGHT THE FIRST TIME!...");
					//topPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex-2];
					//leftPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex-3];
					//topPageReverseImage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex - 1];
					//bottomPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex];
...
- (void) isGoingRight:(BOOL) right {
	...
	NSLog(@"- (void) isGoingRight:(BOOL) right {");
...
- (void) setLayerFrames {
	...
	bottomPageShadow.frame = CGRectMake(leafEdge * rightPageBoundsRect.size.width - 3, 
										0, 
										40, 
										bottomPage.bounds.size.height);
...
- (void) willTurnToPageAtIndex:(NSUInteger)index {
	...
	//[delegate removeViews];	//Takes Views Away

	//[delegate setsAllowedToTurnPage:NO];

	
	NSLog(@"- (void) willTurnToPageAtIndex:(NSUInteger)index {");
...
- (void) didTurnToPageAtIndex:(NSUInteger)index {
	...
	NSLog(@"- (void) didTurnToPageAtIndex:(NSUInteger)index {");

	//[delegate addViewsBack]; //Adds Views Back
...
- (void) didTurnPageBackward {
	NSLog(@"- (void) didTurnPageBackward {");

	//currentPageIndex = 4;
	
	if (didTurn) {
		//NSLog(@"LEFT CPI %i NP %i",currentPageIndex, numberOfPages);
		if (turnType == 2) {
			[delegate setUpPagesTurnedLeftSearch]; 
		}
		else {
			[delegate setUpPagesTurnedLeft]; 
		}
	...
	//self.currentPageIndex = self.currentPageIndex + numberOfVisiblePages - 2;

	
	NSLog(@"CURRENT PAGE INDEX: %i",currentPageIndex);
...
- (void) didTurnPageForward {
	...
	NSLog(@"- (void) didTurnPageForward {");

	if (didTurn) {
		//NSLog(@"RIGHT CPI %i NP %i",currentPageIndex, numberOfPages);
		if (turnType == 2) {
			[delegate setUpPagesTurnedRightSearch]; 
		}
		else {
			[delegate setUpPagesTurnedRight]; 
		}
	}
	else {
		[delegate setUpPages]; 
...
- (void) changeCurrentPageIndex:(int)number {

	NSLog(@"- (void) changeCurrentPageIndex:(int)number {");

	thePageIndex = number;
	currentPageIndex = thePageIndex;
...
- (void) setCurrentPageIndex:(NSUInteger)aCurrentPageIndex {
	...
	NSLog(@"- (void) setCurrentPageIndex:(NSUInteger)aCurrentPageIndex {");
...
- (void) setMode:(LeavesViewMode)newMode {
	...
	NSLog(@"- (void) setMode:(LeavesViewMode)newMode {");
...
- (void) touchBegan:(CGPoint)location{
	...
	NSLog(@"- (void) touchBegan:(CGPoint)location{");
...
- (void) touchMoved:(CGPoint)location{
	...
	NSLog(@"- (void) touchMoved:(CGPoint)location{");
	...
	// Keep the page from going too far left
	if (touchPoint.x < 0) {
		touchPoint.x = 0;
	}
	...
	self.leafEdge = touchPoint.x / self.bounds.size.width;
		if (leafEdge > .9) {
			bottomPageShadow.colors = [NSArray arrayWithObjects:
									   (id)[[[UIColor blackColor] colorWithAlphaComponent:.8] CGColor],
									   (id)[[UIColor clearColor] CGColor],
									   nil];
			topPageShadow.colors = [NSArray arrayWithObjects:
									(id)[[[UIColor blackColor] colorWithAlphaComponent:.8] CGColor],
			...
			bottomPageShadow.colors = [NSArray arrayWithObjects:
									   (id)[[[UIColor blackColor] colorWithAlphaComponent:leafEdge-.1] CGColor],
									   (id)[[UIColor clearColor] CGColor],
									   nil];
			topPageShadow.colors = [NSArray arrayWithObjects:
									(id)[[[UIColor blackColor] colorWithAlphaComponent:leafEdge-.1] CGColor],
			...
			bottomPageShadow.colors = [NSArray arrayWithObjects:
									   (id)[[[UIColor blackColor] colorWithAlphaComponent:.3] CGColor],
									   (id)[[UIColor clearColor] CGColor],
									   nil];
			topPageShadow.colors = [NSArray arrayWithObjects:
									(id)[[[UIColor blackColor] colorWithAlphaComponent:.3] CGColor],
...
- (void) touchEnded:(CGPoint)location{
	...
	NSLog(@"- (void) touchEnded:(CGPoint)location{");
	...
	BOOL dragged = distance(touchPoint, touchBeganPoint) > [self dragThreshold];
		...
		topPageReverseShading.colors = [NSArray arrayWithObjects:
										...
										(id)[[UIColor clearColor] CGColor],
										nil];
		if (touchBeganPoint.x > 512){
			if ((dragged && self.leafEdge <= 0.9) || (!dragged && [self touchedNextPage])){
		...
		else if (touchBeganPoint.x < 512){
			if ((dragged && self.leafEdge >= 0.05) || (!dragged && [self touchedPrevPage])){
				[self willTurnToPageAtIndex:currentPageIndex];
